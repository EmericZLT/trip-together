...
1. 使用邮箱和密码注册，不要求填写昵称。线上注册、登录和忘记密码均使用邮件验证码；本地跳过邮箱验证。注册后进入首页，即使没有行程也可以自由使用底部导航和个人资料。
...
7. 在账本添加支出，选择实际付款人和分摊成员，可上传凭证。待核对预订在确认付款人与实际金额之前不计入账本。支持同行成员修改、删除支出和导出 CSV。
...
- 用户账号独立于行程。`trips` 和 `trip_members` 管理所属关系；事项、清单、预订、支出、旅行文件均关联 `trip_id`。
